...
design will fit onto a given circuit board area. Taking the footprints
of the individual components, it will try to find an arrangement that
accomodates all parts. It will then report whether or not the elements
...
considerations. Consider a "no" answer definitive, but consider a "yes"
...
benches. For instructions, refer to the section `Using Test
...
This analysis assumes that each component that will be placed on the
printed circuit board (PCB) has an *EDA Model* (EAGLE Schematic file)
associated with it. Information about the component's size is taken from
these model files.
